...
/usr/local/bin/rtlamr -unique=true -msgtype=scm -format=json |\
    tee -a /opt/powermeter/backup.json > /dev/null #| \
#    /opt/powermeter/powermeter-sqlite serve -cache /opt/powermeter/power.gob -db /opt/powermeter/power.db -meter 18011759 -http 0.0.0.0:5000
